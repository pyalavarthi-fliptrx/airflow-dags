...
class PostgresListOperator(BaseOperator):
    """
    ...
    def __init__(
        ...
        *,
        ...
        postgres_conn_id: str = 'redshift_connection',
        autocommit: bool = False,
        parameters: Optional[Union[Mapping, Iterable]] = None,
        database: Optional[str] = None,
        runtime_parameters: Optional[Mapping] = None,
        column_to_list: str = None,
        **kwargs,
    ) -> None:
    ...
    def execute(self, context: 'Context'):
        records = []
        if self.column_to_list is None:
            raise TypeError("column_to_list can not be null.")
        ...
        if self.runtime_parameters:
            ...
            df = self.hook.get_pandas_df(sql=final_sql, parameters=self.parameters)
        else:
            df = self.hook.get_pandas_df(sql=self.sql, parameters=self.parameters)
        for output in self.hook.conn.notices:
            self.log.info(output)
        if self.column_to_list is not None:
            return df[self.column_to_list].tolist()